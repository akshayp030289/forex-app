node {
    ...
    stage('Copy files to Docker Host') {
        withCredentials([sshUserPrivateKey(credentialsId: 'ec2-user-Akshay', keyFileVariable: 'KEY', usernameVariable: 'USERNAME')]) {
            withCredentials([string(credentialsId: 'aws_server_akshay', variable: 'AGENT_HOST')]) {
                sh '''
